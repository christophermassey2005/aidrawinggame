...
namespace EXEForCNNPredictv5_ForUserDrawing
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            float confidenceforuserdrawing = predictionsasarray[currentitemindex];
            //Console.WriteLine("confidence is" + confidence.ToString());

            File.WriteAllText("E:/CS Project/imageprediction/confidenceforuserdrawing.txt", String.Empty);

            /*
            using (StreamWriter w = File.AppendText(@"E:/CS Project/imageprediction/confidenceforuserdrawing.txt"))
            {
                w.WriteLine(confidenceforuserdrawing.ToString());
            } 
            */
            string json = File.ReadAllText(@"E:/CS Project/imageprediction/properties.json");
            dynamic jsonObj = Newtonsoft.Json.JsonConvert.DeserializeObject(json);
            //Console.WriteLine(confidence);
            jsonObj["confidenceforuserdrawing"] = confidenceforuserdrawing;
